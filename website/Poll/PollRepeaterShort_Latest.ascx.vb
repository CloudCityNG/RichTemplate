...
Partial Class Polls_PollRepeaterShort_Latest
    ...
    Private Sub LoadLatestPollQuestionAndAnswers()
        ...
        If intMemberID > 0 Then
            dtPollQuestion = If(boolEnableGroupsAndUserAccess, PollDAL.GetPoll_LatestAndAccess_FrontEnd_WithMemberSubmission(MemberDAL.GetCurrentMemberGroupIDs, intMemberID, intSiteID), PollDAL.GetPoll_Latest_FrontEnd_WithMemberSubmission(intMemberID, intSiteID))
        Else
            dtPollQuestion = If(boolEnableGroupsAndUserAccess, PollDAL.GetPoll_LatestAndAccess_FrontEnd(MemberDAL.GetCurrentMemberGroupIDs, intMemberID, intSiteID), PollDAL.GetPoll_Latest_FrontEnd(intSiteID))
        End If
        ...
        If dtPollQuestion.Rows.Count > 0 Then
            ...
            If dtPollAnswers.Rows.Count > 0 Then
                ...
                'set the id to the rblPoll answers so the check boxes don't get cached when refreshing page
                ...
                If Not IsPostBack Then
                    ...
                    If intMemberID > 0 Then
                        ...
                        If Not drPollQuestion("PollAnswerID") Is DBNull.Value Then
                            ...
                            btnVote.Text = Resources.Poll_FrontEnd.Poll_PollRepeaterShort_Latest_VoteUpdate
                        Else
                            'user has not voted on this poll, so set the vote button text
                            btnVote.Text = Resources.Poll_FrontEnd.Poll_PollRepeaterShort_Latest_VoteAdd
                        End If
                    End If
            ...
            If rblPollAnswers.SelectedValue.Length > 0 Then
                ...
                If dtPollAnswer.Rows.Count > 0 Then
                    ...
                    If dtPoll.Rows.Count > 0 Then
                        ...
                        litPollSubmissionMsg.Text = "<b>" + Resources.Poll_FrontEnd.Poll_PollRepeaterShort_Latest_SuccessMessage + "</b><br/>"
                    ...
                    Else
                        ...
                        litPollSubmissionMsg.Text = "<b>" + Resources.Poll_FrontEnd.Poll_PollRepeaterShort_Latest_ErrorQuestionAnswerMessage + "</b><br/>"
                        litPollSubmissionMsg.Visible = True
                    End If

                Else
                    'User has choosen a non-existant answer (probably from hacking)
                    litPollSubmissionMsg.Text = "<b>" + Resources.Poll_FrontEnd.Poll_PollRepeaterShort_Latest_ErrorAnswerMessage + "</b><br/>"
                    litPollSubmissionMsg.Visible = True
                End If

            Else
                litPollSubmissionMsg.Text = "<b>" + Resources.Poll_FrontEnd.Poll_PollRepeaterShort_Latest_ErrorSelectAnswer + "</b><br/>"
                litPollSubmissionMsg.Visible = True
            End If
        Else
            litPollSubmissionMsg.Text = Resources.Poll_FrontEnd.Poll_PollRepeaterShort_Latest_LoginToVote + "<br/><a href='/login/'>" + Resources.Poll_FrontEnd.Poll_PollRepeaterShort_Latest_VoteSubmitted_ClickHereToLogin + "</a><br/>"
            litPollSubmissionMsg.Visible = True
        End If
