...
Partial Class Interior
    ...
    Protected Sub DisplayMainContent()
        ...
        If dtWebInfo.Rows.Count > 0 Then
            ...
            End If
        Else
            'If the Page exists redirect them to the login page, otherwise the page is not found
            '=> REMEMBER that users can see all pages, and get sent to the login page if they don't have access to this page
            '=> So rather than send them to the 'page-not-found' page if they do not have access we send them to the login page
            Dim bPageExists As Boolean = (WebInfoDAL.GetWebInfo_ByID_FrontEnd(PageID).Rows.Count > 0)
            If intMemberID = 0 AndAlso bPageExists Then
                Response.Redirect("/Login/")
            ElseIf bPageExists Then
                Response.Redirect("/AccessDenined.aspx")
            Else
                Response.Redirect("/404.aspx")
            End If
        End If
