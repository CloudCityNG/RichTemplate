...
<!--#INCLUDE Virtual="/editor2/assetmanager/sessioncheckhome.asp"-->
<!--#INCLUDE VIRTUAL ="/admin/tagsmodule/tagConfig.inc"-->	

<%
'Delete any tags where files have been removed
SET CON = SERVER.CREATEOBJECT("ADODB.CONNECTION")
CON.OPEN ConnectionString

	getFileID = "Select * from ss_modules_searchtags_xref where recordSetID = -1"
	...
	Do While Not RS.EOF
		...
		getFileID2 = "Select * from fileXref where fileID = "&RS("recordID")&""
		'response.write getFileID2
		Set RS2 = Con.execute(getFileID2)
		If RS2.EOF then
		'If you find a link in taqXref, but not in fileXref, delete the tagxref
			
			delTag = "Delete From ss_modules_searchtags_xref where searchTagXrefID = "&RS("tagXrefID")&""
			'response.write deltag
			Con.execute(delTag)
...
function recursive(oFolder,oFolder_base,sName)
	...
	'remove frontpage folders
    if InStr(1,oFolder.path,"_vti_cnf")=0 and InStr(1,oFolder.path,"_derived")=0 then
...
function writeFileSelections()
	...
	for each oFile in oFiles
		...
		if bDisplay then
			...
			If LCase(sExt)="docx" then sIcon="ico_doc.gif"
			If LCase(sExt)="doc" then sIcon="ico_doc.gif"
			...
			If LCase(sExt)="xlsx" then sIcon="ico_xls.gif"
			...
			Response.Write "<tr style='background:" & sColorResult & "'>" & VbCrLf & _
				"<td width='20px'><img src='images/"&sIcon&"'></td>" & VbCrLf & _
				"<td valign=top><u id=""idFile"&nIndex&""" style='cursor:pointer;' onclick=""selectFile(" & nIndex & ")"">" & oFile.name & "</u>&nbsp;&nbsp;<img style='cursor:pointer;' onclick=""downloadFile(" & nIndex & ")"" src='download.gif'></td>" & VbCrLf & _
				...
				"<td width= '30px' valign=top nowrap onclick=""deleteFile(" & nIndex & ")""><u style='font-size:10px;cursor:pointer;color:crimson' " & sFolderAdmin & ">" & VbCrLf
...
rp = lcase(sFile_VirtualPath)
rp = replace(rp, lcase(oFile.name),"")

SET CON = SERVER.CREATEOBJECT("ADODB.CONNECTION")
CON.OPEN ConnectionString

	getFileID = "Select * from fileXref where fileName = '"&oFile.name&"'"
	'Response.write getFileID
	set RS = Con.execute(getFileID)
	
	If Not RS.EOF then

		getTags = "Select * from ss_modules_searchtags_xref where recordID = "&RS("fileID") & " and recordSetID = -1"
		set tagRS = con.execute(getTags)
		If tagRS.EOF then
			Response.write "</u></td><td width='80px'><a href='/admin/modules/searchtags/legacy/?recordID="&RS("fileID")&"&recordSetID=-1&rp=" & rp & "'>Add Tags</a></td></tr>" & VbCrLf 
		Else
			Response.Write "</u></td><td width='80px'><a href='/admin/modules/searchtags/legacy/?recordID="&RS("fileID")&"&recordSetID=-1&rp=" & rp & "'>Edit Tags</a></td></tr>" & VbCrLf
		End If
...
'CON.CLOSE
			...
			'Response.Write "</u></td><td width='80px'><a href=''>Add Tags</a></td></tr>" & VbCrLf	
End if
...
function changeFolder()
	...
	Form1.action=getAction()
	//document.write(document.getElementById("selCurrFolder").value)
	//document.write(document.getElementById("inpFile").value)
	Form1.submit();
	}
...
function downloadFile(index)
	{
	document.write (document.getElementById("inpFile"+index).value)
...
function deleteFile(index)
	...
	if (confirm(getTxt("Delete this file ?")) == true) 
		...
		Form1.elements.oFileName.value = "<%=session("oFileName")%>";
		Form1.action=getAction()
		Form1.submit();
		}
	}
...
<%PNAME = "RichTemplate Image/Document Library"%>
...
<table width="100%" height="90%" align=center style="" cellpadding=0 cellspacing=0 border=0 >
<tr>
<td valign=top style="background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
		...
		<form method=post name="Form1" id="Form1">
				...
				<input type="hidden" name="oFileName" ID="oFileName">
		</form>
		...
		<td valign=top align="center">
				...
				<tr>
				<td>
					<div id="idPreview" style="text-align:center;overflow:auto;width:600px;height:245;border:#d7d7d7 5px solid;border-bottom:#d7d7d7 3px solid;background:#ffffff;margin-right:2;"></div>
					<div align=center><input type="hidden" id="inpSource" name="inpSource" style="border:#cfcfcf 1px solid;width:600" class="inpTxt"></div>
				</td>
		...
		<td>
				<div <%=sFolderAdmin%>>
					...
					<span name="txtLang" id="txtLang">Upload File</span>: 
					<input type="file" id="File1" name="File1" class="inpTxt" style="height: 21px">
